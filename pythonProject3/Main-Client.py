...
def start_screen_sharing():
    run_as_admin("screen-client.py")
def start_ftp():
    subprocess.Popen(['python', 'FTPclient.py'])
root = tk.Tk()
...
button_chat = tk.Button(frame, text="Connect to chat", command=start_chat, **style)
button_chat.grid(row=0, column=0, padx=10, pady=10)

button_screen_sharing = tk.Button(frame, text="Connect to Screen Sharing", command=start_screen_sharing, **style)
button_screen_sharing.grid(row=1, column=0, padx=10, pady=10)

button_ftp = tk.Button(frame, text="Connect to FTP", command=start_ftp, **style)
button_ftp.grid(row=2, column=0, padx=10, pady=10)

button_exit = tk.Button(frame, text="Exit", command=root.quit, **style)
